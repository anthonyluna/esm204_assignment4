...
3. ADD NARRATIVES FOR THE OTHER SECTIONS
4. COPY NARATIVE OVER FROM THE OTHER DOCUMENT
...
consumption_low_high_g <- function(t){econ_retained_expected(t)*exp(g_high*t)}
...
```{r 1becon}
ggplot()+
	stat_function(fun=econ_retained_low,aes(x=0,color="No Climate Change"))+
	stat_function(fun=econ_retained_med,aes(x=0,color="Modest Climate Change"))+
	stat_function(fun=econ_retained_high,aes(x=0,color="Extreme Climate Change"))+
	xlim(0,200)+scale_y_continuous(breaks=seq(from=0,to=1.1,by=.1),limits = c(0,1.1))
```
...
```{r 2a}
pv_utility_low <- sum(map_dbl(seq(from=0,to=199,by=1),function(x){utility_low(x)/(1+r_expected)^x}))
pv_utility_expected <- sum(map_dbl(seq(from=0,to=199,by=1),function(x){utility_expected(x)/(1+r_expected)^x}))
...
```{r 2b}
pv_utility_expected_high_T    <- sum(map_dbl(seq(from=0,to=199,by=1),function(x){utility_expected_high_T(x)/(1+r_expected_high_T)^x}))
pv_utility_expected_high_beta <- sum(map_dbl(seq(from=0,to=199,by=1),function(x){utility_expected_high_beta(x)/(1+r_expected_high_beta)^x}))
pv_utility_expected_high_g    <- sum(map_dbl(seq(from=0,to=199,by=1),function(x){utility_expected_high_g(x)/(1+r_expected_high_g)^x}))
pv_utility_expected_high_nu   <- sum(map_dbl(seq(from=0,to=199,by=1),function(x){utility_expected_high_nu(x)/(1+r_expected_high_nu)^x}))

pv_utility_low_high_beta <- sum(map_dbl(seq(from=0,to=199,by=1),function(x){utility_low_high_beta(x)/(1+r_expected_high_beta)^x}))
pv_utility_low_high_g    <- sum(map_dbl(seq(from=0,to=199,by=1),function(x){utility_low_high_g(x)/(1+r_expected_high_g)^x}))
pv_utility_low_high_nu   <- sum(map_dbl(seq(from=0,to=199,by=1),function(x){utility_low_high_nu(x)/(1+r_expected_high_nu)^x}))
...
```{r 2d}
pv_utility_uncertain <- sum(map_dbl(seq(from=0,to=199,by=1),function(x){utility_uncertain(x)/(1+r_expected)^x}))
